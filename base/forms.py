...
from .models import Choix, Client, Fournisseur, Produit, QuizzM, Joueur, GestioncM, VEntrepriseM
#from django.contrib.auth.forms import UserCreationForm

class createPlayerForm(ModelForm):
    class Meta:
        model = Joueur
        fields = ["username", "password1", "password2", "points", "type_de_missions", "nombre_de_missions"]
        widgets = {
...
class createQuizzForm(ModelForm):
    def __init__(self, *args, **kwargs):
            ...
            self.fields['type_de_quizz'].queryset = self.fields['type_de_quizz'].queryset.exclude(choix__in=[p for p in Choix.objects.all() if p.choix in ["vie d'entreprise", "gestion-commerciale"]])
...
class createVEntrepriseForm(ModelForm):
    class Meta:
        model = VEntrepriseM
        fields = '__all__'
        exclude = ['type']
